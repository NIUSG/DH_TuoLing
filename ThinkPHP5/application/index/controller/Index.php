...
class Index extends Common
{
    ...
    private $blog_home_list_key = "blog_home_list_key";
    private $blog_click_key = "blog_click_key";
    private $blog_new_key = "blog_new_key";
    private $link_often_key = "link_often_key";
...
$this->info("[write_db][{$write_db}][blog_list][{$blog_list}][blog_click][{$blog_click}][blog_new][{$blog_new}][link_often][{$link_often}]");
    ...
    {
        if($this->is_cache && $this->file_obj->has($this->blog_home_list_key)){
            $res = $this->file_obj->get($this->blog_home_list_key);
        }else{
            $sql = "SELECT bloginfo_id,bloginfo_title,bloginfo_describe,bloginfo_img,from_unixtime(bloginfo_createtime,'%Y-%m-%d') bloginfo_createtime,class_title
                ...
                LIMIT 10";
            $res = Db::query($sql);
            $this->file_obj->set($this->blog_home_list_key,$res,$this->cache_time);
        }
        return $res;
    }
    //点击排行技术类博客前十条,并且加载
    ...
    {
        if($this->is_cache && $this->file_obj->has($this->blog_click_key)){
            $res = $this->file_obj->get($this->blog_click_key);
        }else{
            $sql = "SELECT bloginfo_id,bloginfo_title,bloginfo_click
                ...
                LIMIT 10";
            $res = Db::query($sql);
            $this->file_obj->set($this->blog_click_key,$res,$this->cache_time);
        }
    ...
    {
        if($this->is_cache && $this->file_obj->has($this->blog_new_key)){
            $res = $this->file_obj->get($this->blog_new_key);
        }else{
            $sql = "SELECT bloginfo_id,bloginfo_title,bloginfo_click
                FROM ns_bloginfo
                WHERE bloginfo_status = 1 and class_id IN (SELECT class_id
                ...
                LIMIT 5";
            $res = Db::query($sql);
            $this->file_obj->set($this->blog_new_key,$res,$this->cache_time);
        }
    ...
    {
        if($this->is_cache && $this->file_obj->has($this->link_often_key)){
            $res = $this->file_obj->get($this->link_often_key);
        }else{
            $sql = "SELECT link_id,link_title,link_url
                ...
                LIMIT 20";
            $res = Db::query($sql);
            $this->file_obj->set($this->link_often_key,$res,$this->cache_time);
        }
